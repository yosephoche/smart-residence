...
export default function HouseTypesPage() {
  ...
  const handleDeleteClick = (type: HouseType) => {
    ...
    if (housesWithType.length > 0) {
      setErrorMessage(
        `Cannot delete "${type.typeName}" because ${housesWithType.length} house(s) are using this type. Please reassign or delete those houses first.`
      );
  ...
  const handleDeleteConfirm = async () => {
    ...
    if (res.ok) {
      setSuccessMessage(`House type "${typeToDelete.typeName}" has been deleted successfully`);
      ...
      setErrorMessage(err.error || "Failed to delete house type. It may still be in use.");
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Manage house types and monthly IPL rates
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-success-50 rounded-xl border-2 border-success-200 p-4 shadow-sm">
          <p className="text-sm font-medium text-success-700">Houses Using Types</p>
      ...
      <div className="bg-white rounded-xl border-2 border-gray-200 p-4">
        ...
        <div className="mt-4 pt-4 border-t-2 border-gray-100">
          <span className="text-sm text-gray-600">
            Showing <span className="font-semibold">{filteredTypes.length}</span> of{" "}
            <span className="font-semibold">{houseTypes.length}</span> types
          </span>
        </div>
      </div>
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to delete "${typeToDelete?.typeName}"? This action cannot be undone.`}
        confirmText="Delete"
        variant="danger"
