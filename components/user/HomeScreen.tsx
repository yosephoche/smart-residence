  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (financialResult.status === "fulfilled")
        setFinancialData(financialResult.value);
      if (
        onDutyResult.status === "fulfilled" &&
        Array.isArray(onDutyResult.value)
      ) {
        setOnDutyStaff(onDutyResult.value);
      }
      if (
        urgentContactsResult.status === "fulfilled" &&
        Array.isArray(urgentContactsResult.value)
      ) {
        setUrgentContacts(urgentContactsResult.value);
      }
      if (
        waTemplateResult.status === "fulfilled" &&
        waTemplateResult.value?.template
      ) {
        ...
        {onDutyStaff.length === 0 ? (
          ...
          <div className="space-y-0">
            {onDutyStaff.map((item, index) => {
              const clockInTime = new Date(item.clockInAt).toLocaleTimeString(
                "id-ID",
                {
                  hour: "2-digit",
                  minute: "2-digit",
          ...
          <div className="w-full bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-amber-600 mt-0.5">
                Bukti bayar sedang diproses admin
              </p>
            </div>
          </div>
        ) : (
              ...
              <div key={payment.id}>
                <div className="flex items-center justify-between py-3">
                  ...
                  <div className="text-right">
                    ...
                    <span
                      className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${statusConf.bg} ${statusConf.color}`}
                    >
                      {statusConf.label}
                    </span>
                  </div>
